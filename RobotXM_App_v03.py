# =============================================================================
#  ROBOT XM - VERSIÓN DE PRODUCCIÓN (FINAL + FILTRO VERSIÓN)
...
#  Corrección 2: Los reportes de Excel ahora guardan los números como números.
#  Mejora 3: Opción de filtrar por Versión específica (tx1, txr, etc.) en Reportes.
# =============================================================================
...
# =============================================================================
#  MÓDULO 3: GENERACIÓN DE REPORTES (CON SELECCIÓN DE VERSIÓN)
# =============================================================================
...
def generar_reporte_logica(config):
    ...
    for item in config['filtros_reporte']:
        tareas_a_procesar.append({
            ...
            'filtro_version': item.get('version') # NUEVO CAMPO
        })
    ...
    try:
        with pd.ExcelWriter(ruta_reporte_completa, engine='openpyxl') as writer:
            ...
            for tarea in tareas_a_procesar:
                ...
                ver_filtro_usuario = tarea['filtro_version'] # Leer versión solicitada
                ...
                if col_filtro_usuario and val_filtro_usuario:
                    ...
                    if nombre_columna_real:
                        ...
                        print(f"   ⚠️ Campo '{col_filtro_usuario}' no existe en tabla.")

                # 2. Filtro de Versión (NUEVO)
                ...
                try:
                    ...
                    # --- LÓGICA DE VERSIÓN ---
                    if ver_filtro_usuario:
                        # Si el usuario pidió una versión específica, NO aplicamos lógica de pesos.
                        # Solo ordenamos por fecha.
                        df_final = df.sort_values(by='Fecha', ascending=True)
                    else:
                        # Lógica original (Mejor versión automática)
...
class AplicacionXM:
    ...
    def crear_tab_filtros(self):
        ...
        # --- ACTUALIZAR COLUMNAS PARA MOSTRAR VERSIÓN ---
        ...
        self.tree_filtros.heading("Ver", text="Versión"); self.tree_filtros.column("Ver", width=60) # Nueva columna
    ...
    def add_filtro(self):
        if self.ent_r_tab.get():
            # Agregamos la versión al insertar
            self.tree_filtros.insert("", "end", values=(
                self.ent_r_tab.get(), 
                self.ent_r_cam.get(), 
                self.ent_r_val.get(),
                self.ent_r_ver.get() # Tomar valor versión
            ))
            # Limpiar campos
            self.ent_r_tab.delete(0, tk.END)
            self.ent_r_cam.delete(0, tk.END)
            self.ent_r_val.delete(0, tk.END)
            self.ent_r_ver.delete(0, tk.END)
    ...
    def get_config(self):
        return {
            ...
            'filtros_reporte': [{
                ...
                'version': str(self.tree_filtros.item(i)['values'][3]) # Guardar versión
