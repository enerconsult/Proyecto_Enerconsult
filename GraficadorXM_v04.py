...
class ModuloVisualizador:
    def __init__(self, root):
        ...
        self.var_version = tk.StringVar() # <--- NUEVA VARIABLE PARA VERSIÓN
        ...
        # FILA 1: Filtros (Campo y Valor)
        ttk.Label(frame_cfg, text="3. Campo Clasificador:").grid(row=1, column=0, sticky="w", pady=5)
        self.cb_campo_filtro = ttk.Combobox(frame_cfg, textvariable=self.var_campo_filtro, state="readonly", width=25)
        self.cb_campo_filtro.grid(row=1, column=1, padx=5)
        self.cb_campo_filtro.bind("<<ComboboxSelected>>", self.al_seleccionar_campo_filtro)

        ttk.Label(frame_cfg, text="4. Valor Específico:").grid(row=1, column=2, sticky="w", padx=10)
        self.cb_valor_filtro = ttk.Combobox(frame_cfg, textvariable=self.var_valor_filtro, state="readonly", width=25)
        self.cb_valor_filtro.grid(row=1, column=3, padx=5)

        # FILA 2: Checkbox 24h y Columna Valor
        ...
        self.chk_horario.grid(row=2, column=0, columnspan=2, sticky="w", pady=5)

        self.lbl_valor = ttk.Label(frame_cfg, text="6. Columna Valor Único:")
        self.lbl_valor.grid(row=2, column=2, sticky="w", padx=10)
        self.cb_campo_valor = ttk.Combobox(frame_cfg, textvariable=self.var_campo_valor, state="readonly", width=25)
        self.cb_campo_valor.grid(row=2, column=3, padx=5)

        # FILA 3: Operación
        ttk.Label(frame_cfg, text="7. Operación Matemática:").grid(row=3, column=0, sticky="w", pady=5)
        ...
        self.cb_agregacion.grid(row=3, column=1, padx=5)

        # FILA 4: Fechas
        ttk.Label(frame_cfg, text="Fecha Inicio (YYYY-MM-DD):").grid(row=4, column=0, sticky="w", pady=10)
        self.ent_fecha_ini = ttk.Entry(frame_cfg, textvariable=self.var_fecha_ini, width=25)
        self.ent_fecha_ini.grid(row=4, column=1, padx=5)
        self.ent_fecha_ini.insert(0, "2020-01-01") 

        ttk.Label(frame_cfg, text="Fecha Fin (YYYY-MM-DD):").grid(row=4, column=2, sticky="w", padx=10)
        self.ent_fecha_fin = ttk.Entry(frame_cfg, textvariable=self.var_fecha_fin, width=25)
        self.ent_fecha_fin.grid(row=4, column=3, padx=5)
        self.ent_fecha_fin.insert(0, datetime.today().strftime('%Y-%m-%d')) 

        # Botón Graficar
        ttk.Button(frame_cfg, text="📊 GENERAR VISUALIZACIÓN", command=self.generar_grafico).grid(row=5, column=0, columnspan=4, pady=15, sticky="ew")
    ...
    def al_seleccionar_tabla(self, event):
        ...
        # --- NUEVA LÓGICA: CARGAR VERSIONES ---
        # Buscamos si existe la columna 'version_dato'
        if 'version_dato' in cols:
            try:
                ...
                if lista_versiones:
                    # Preferencia por 'txr' si existe, sino el primero
                    if 'txr' in lista_versiones:
                        self.cb_version.set('txr')
                    else:
        ...
        # Filtros (Ignoramos columnas técnicas y horarias)
    ...
    def generar_grafico(self):
        tabla = self.var_tabla.get()
        version = self.var_version.get() # <--- OBTENER VERSIÓN
        ...
        try:
            ...
            query = f"SELECT * FROM {tabla} WHERE 1=1" # Truco para concatenar ANDs
            
            # 1. Filtro por Campo Clasificador
            if campo_filtro and valor_filtro:
                query += f" AND CAST({campo_filtro} AS TEXT) = '{valor_filtro}'"
            
            # 2. Filtro por Versión (Si aplica)
            ...
            if df.empty:
                messagebox.showinfo("Vacío", f"No hay datos para {version} con esos filtros.")
                return
            ...
            # Título del Gráfico con la Versión
            titulo_grafico = f"{tabla.upper()} - {valor_filtro} - Ver: {version} ({operacion})"
    ...
    def dibujar_plot(self, serie, titulo):
        ...
        line, = ax.plot(serie.index, serie.values, marker='o', linestyle='-', markersize=4, color='#2980b9') 
