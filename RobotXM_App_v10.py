# =============================================================================
#  ROBOT XM - SUITE INTEGRADA (v14 - FINAL + LAYOUT OPTIMIZADO)
#  Funcionalidades:
#  1. Descarga FTP Automática.
#  2. Base de Datos SQLite.
#  3. Reportes Excel.
#  4. VISUALIZADOR AVANZADO (Layout en 3 Columnas para ahorrar espacio).
#  5. Logo Corporativo.
# =============================================================================
...
class ModuloVisualizador:
    def __init__(self, parent_frame, config):
        ...
        self.cb_campo_filtro1 = ttk.Combobox(col1, textvariable=self.var_campo_filtro1, state="readonly", width=18)
        self.cb_campo_filtro1.grid(row=2, column=1, padx=2)
        self.cb_campo_filtro1.bind("<<ComboboxSelected>>", self.al_seleccionar_campo_filtro1)
        self.cb_valor_filtro1 = ttk.Combobox(col1, textvariable=self.var_valor_filtro1, width=18) 
        self.cb_valor_filtro1.grid(row=3, column=1, padx=2)

        ttk.Label(col1, text="Filtro 2 (opc):").grid(row=4, column=0, sticky="w", pady=2)
        self.cb_campo_filtro2 = ttk.Combobox(col1, textvariable=self.var_campo_filtro2, state="readonly", width=18)
        self.cb_campo_filtro2.grid(row=4, column=1, padx=2)
        self.cb_campo_filtro2.bind("<<ComboboxSelected>>", self.al_seleccionar_campo_filtro2)
        self.cb_valor_filtro2 = ttk.Combobox(col1, textvariable=self.var_valor_filtro2, width=18) 
        self.cb_valor_filtro2.grid(row=5, column=1, padx=2)

        # COLUMNA 2: CONFIGURACIÓN
        col2 = ttk.LabelFrame(frame_controls, text="2. Configuración")
        col2.pack(side="left", fill="both", expand=True, padx=5)

        self.chk_horario = ttk.Checkbutton(col2, text="Es Horaria (24h)", variable=self.var_es_horario, command=self.toggle_campo_valor)
        self.chk_horario.grid(row=0, column=0, columnspan=2, sticky="w", pady=2)

        self.lbl_valor = ttk.Label(col2, text="Variable:")
        self.lbl_valor.grid(row=1, column=0, sticky="w", pady=2)
        self.cb_campo_valor = ttk.Combobox(col2, textvariable=self.var_campo_valor, state="readonly", width=18)
        self.cb_campo_valor.grid(row=1, column=1, padx=2)
        ...
        ttk.Button(col3, text="📊 GRAFICAR", command=self.generar_grafico).grid(row=3, column=0, columnspan=2, pady=15, sticky="ew", padx=10)
        ttk.Button(col3, text="📥 EXCEL", command=self.exportar_datos_excel).grid(row=4, column=0, columnspan=2, pady=5, sticky="ew", padx=10)
    ...
    def toggle_campo_valor(self):
        if self.var_es_horario.get():
            self.cb_campo_valor.configure(state="disabled"); self.lbl_valor.configure(text="(Modo horario)")
        else:
            self.cb_campo_valor.configure(state="readonly"); self.lbl_valor.configure(text="Variable:")
