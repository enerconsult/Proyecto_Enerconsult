...
MINTY_PALETTE = {
    ...
    "secondary": "#F3969A", # Salmon
    "success": "#56CC9D",   # Green
    "info": "#6CC3D5",      # Cyan
    "warning": "#FFCE67",   # Yellow
    "danger": "#FF7851",    # Orange
    "light": "#F8F9FA",     # Off-white
    "dark": "#5A5A5A",      # Gray Text
...
class CustomDropdownWithTooltip:
    ...
    def show_dropdown(self, event=None):
        ...
        self.listbox = tk.Listbox(frame_list, width=chars_w, height=8, yscrollcommand=scrollbar.set,
                                  exportselection=False, bg=MINTY_PALETTE["bg_card"], fg=MINTY_PALETTE["dark"],
                                  selectbackground=MINTY_PALETTE["primary"], selectforeground="#ffffff",
...
class CalendarDialog(tk.Toplevel):
    def __init__(self, parent, callback):
        ...
        # Posicionar con ajuste de bordes
        try:
            x_mouse = parent.winfo_pointerx()
            y_mouse = parent.winfo_pointery()
            
            # Dimensiones estimadas del dialogo (ya que aun no se renderiza)
            w_dlg = 260
            h_dlg = 280
    ...
    def setup_ui(self):
        ...
        btn_prev = tk.Button(header_frame, text="<", command=self.prev_month, 
                             bg=MINTY_PALETTE["primary"], fg="white", relief="flat", font=("Segoe UI", 9, "bold"))
        btn_prev.pack(side="left")
        
        self.lbl_month_year = tk.Label(header_frame, text="", width=15, font=("Segoe UI", 10, "bold"), bg="#ffffff", fg=MINTY_PALETTE["dark"])
        self.lbl_month_year.pack(side="left", expand=True)
        
        btn_next = tk.Button(header_frame, text=">", command=self.next_month, 
                             bg=MINTY_PALETTE["primary"], fg="white", relief="flat", font=("Segoe UI", 9, "bold"))
        btn_next.pack(side="right")
        
        days_frame = tk.Frame(self, bg="#ffffff")
        days_frame.pack()
        days_es = ["Lu", "Ma", "Mi", "Ju", "Vi", "Sá", "Do"]
        for i, d in enumerate(days_es):
            tk.Label(days_frame, text=d, font=("Segoe UI", 8, "bold"), bg="#ffffff", fg=MINTY_PALETTE["secondary"], width=4).grid(row=0, column=i)
    ...
    def build_calendar(self):
        ...
        # Calendar.month_name is locale dependent, generally English if default. 
        # Manual spanish mapping for safety
        ...
        for r, week in enumerate(cal):
            for c, day in enumerate(week):
                ...
                bg_color = "#f1f5f9"
                fg_color = MINTY_PALETTE["dark"]
                if day == date.today().day and self.month == date.today().month and self.year == date.today().year:
                    bg_color = MINTY_PALETTE["info"]
                    fg_color = "white"
                
                btn = tk.Button(self.calendar_frame, text=str(day), width=4, relief="flat", bg=bg_color, fg=fg_color,
    ...
    def select_date(self, day):
        ...
        if self.variable: self.variable.set(self.state)
        self.draw()
        if self.command: self.command()

class ToggleSwitch(tk.Canvas):
    def __init__(self, parent, variable=None, command=None, width=46, height=22, 
                 bg_off="#cbd5e1", bg_on=MINTY_PALETTE["primary"], circle_color="white", bg_parent=MINTY_PALETTE["bg_card"]):
        super().__init__(parent, width=width, height=height, highlightthickness=0, bg=bg_parent)
        ...
        self.state = False
        if self.variable: self.state = self.variable.get()
    ...
    def draw(self):
        ...
        pad = 3
        d = self.height - (pad*2)
        x = self.width - d - pad if self.state else pad
        self.create_oval(x, pad, x+d, pad+d, fill=self.circle_color, outline=self.circle_color)
...
class Card(ttk.Frame):
    def __init__(self, parent, title=None, icon=None, *args, **kwargs):
        self.shadow_frame = tk.Frame(parent, bg=MINTY_PALETTE["info"]) # Color de borde/sombra
        self.mid_frame = tk.Frame(self.shadow_frame, bg=MINTY_PALETTE["bg_card"])
                ...
                try:
                    ...
                    for col in df.columns:
                        if col not in cols_no_num and df[col].dtype == 'object':
                            # Hybrid conversion: Num -> Num, Text -> Text
                            try:
...
class ModuloVisualizador:
    def __init__(self, parent_frame, config):
        ...
        def crear_navegador_fecha(parent, var_fecha, row_idx):
            ...
            tk.Button(f_nav, text="📅", cursor="hand2", relief="flat", bg=MINTY_PALETTE["light"], fg=MINTY_PALETTE["primary"],
        ...
        # Toggle Modo Unico
        fr_toggle = tk.Frame(col3, bg=MINTY_PALETTE["bg_card"])
        fr_toggle.grid(row=2, column=0, columnspan=2, sticky="w", padx=5, pady=5)
        
        ToggleSwitch(fr_toggle, variable=self.var_dia_unico, command=toggle_dia_unico, bg_parent=MINTY_PALETTE["bg_card"]).pack(side="left")
        ttk.Label(fr_toggle, text="Solo un día", background=MINTY_PALETTE["bg_card"]).pack(side="left", padx=5) 
...
class AplicacionXM:
    def __init__(self, root):
        ...
        self.tab_general = tk.Frame(tab_control, bg=MINTY_PALETTE["bg_main"])
        self.tab_archivos = tk.Frame(tab_control, bg=MINTY_PALETTE["bg_main"])
        self.tab_filtros = tk.Frame(tab_control, bg=MINTY_PALETTE["bg_main"])
        self.tab_visualizador = tk.Frame(tab_control, bg=MINTY_PALETTE["bg_main"])
    ...
    def configurar_estilos_modernos(self):
        ...
        c_azul_primario = MINTY_PALETTE["primary"]; c_fondo_principal = MINTY_PALETTE["bg_main"]; c_texto_primario = MINTY_PALETTE["dark"]
        self.root.configure(bg=c_fondo_principal)
        style.configure("TLabel", font=("Segoe UI Semibold", 10), background=MINTY_PALETTE["bg_card"], foreground=MINTY_PALETTE["dark"])
        ...
        style.configure("TLabelframe", background=MINTY_PALETTE["bg_card"], borderwidth=1, relief="solid", bordercolor=MINTY_PALETTE["info"])
        style.configure("TLabelframe.Label", background=MINTY_PALETTE["bg_card"], foreground=c_azul_primario, font=("Segoe UI", 12, "bold"))
        style.configure("TNotebook", background=c_fondo_principal, borderwidth=0, tabmargins=[0, 0, 0, 0], relief="flat")
        style.configure("TNotebook.Tab", padding=[20, 12], font=("Segoe UI Semibold", 11), background=MINTY_PALETTE["light"], foreground=MINTY_PALETTE["dark"], borderwidth=0)
        style.map("TNotebook.Tab", background=[("selected", MINTY_PALETTE["bg_card"]), ("active", MINTY_PALETTE["light"])], foreground=[("selected", c_azul_primario), ("active", c_azul_primario)], expand=[("selected", [0, 0, 0, 0])])
        ...
        style.map("Primary.TButton", background=[("active", "#5AAF9A"), ("pressed", "#489C87"), ("disabled", "#cbd5e1")])
        ...
        style.map("Small.Primary.TButton", background=[("active", "#5AAF9A"), ("pressed", "#489C87"), ("disabled", "#cbd5e1")])
        style.configure("Success.TButton", font=("Segoe UI", 11, "bold"), background=MINTY_PALETTE["success"], foreground="white", borderwidth=0, focuscolor="none", padding=[18, 14])
        style.map("Success.TButton", background=[("active", "#45B889"), ("pressed", "#34A475")])
        style.configure("Danger.TButton", font=("Segoe UI", 11, "bold"), background=MINTY_PALETTE["danger"], foreground="white", borderwidth=0, padding=[18, 14])
        style.map("Danger.TButton", background=[("active", "#E66A46")])
        style.configure("Treeview", background="#ffffff", fieldbackground="#ffffff", rowheight=32, font=("Segoe UI", 11), borderwidth=1, relief="solid", bordercolor=MINTY_PALETTE["info"])
        style.configure("Treeview.Heading", font=("Segoe UI", 11, "bold"), background=MINTY_PALETTE["light"], padding=[12, 8], relief="flat", foreground=MINTY_PALETTE["dark"])
        style.map("Treeview", background=[("selected", c_azul_primario)], foreground=[("selected", "white")])
        style.configure("TEntry", padding=[12, 10], relief="flat", borderwidth=1, bordercolor=MINTY_PALETTE["info"], fieldbackground="#ffffff")
        style.map("TEntry", bordercolor=[("focus", c_azul_primario)])
        style.configure("Compact.TEntry", padding=[5, 2], relief="flat", borderwidth=1, bordercolor=MINTY_PALETTE["info"], fieldbackground="#ffffff")
        style.map("Compact.TEntry", bordercolor=[("focus", c_azul_primario)])
        style.configure("Card.TFrame", background="#ffffff", relief="flat", borderwidth=1, bordercolor=MINTY_PALETTE["info"])
        style.configure("CardHeader.TFrame", background="#fafbfc") # Leave light gray or use light palette
    ...
    def construir_encabezado_logo(self):
        frame_header = tk.Frame(self.root, bg=MINTY_PALETTE["light"], height=70)
        frame_header.pack(fill="x", side="top")
        frame_content = tk.Frame(frame_header, bg=MINTY_PALETTE["light"])
        ...
        if TIENE_PILLOW and os.path.exists(ruta_logo):
            try:
                ...
                lbl_logo = tk.Label(frame_content, image=self.logo_img, bg=MINTY_PALETTE["light"])
                lbl_logo.pack(side="left", padx=20)
                lbl_title = tk.Label(frame_content, text="-    Suite Inteligente", bg=MINTY_PALETTE["light"], fg=MINTY_PALETTE["primary"], font=("Segoe UI", 16, "bold"))
                lbl_title.pack(side="left", padx=15)
            except: pass
        
        tk.Frame(self.root, bg=MINTY_PALETTE["info"], height=1).pack(fill="x", side="top")

    def crear_tab_general(self):
        main_container = tk.Frame(self.tab_general, bg=MINTY_PALETTE["bg_main"])
        ...
        ttk.Label(c_content, text="Credenciales FTP y Rutas", font=("Segoe UI", 12, "bold"), foreground=MINTY_PALETTE["primary"], background=MINTY_PALETTE["bg_card"]).grid(row=0, column=0, columnspan=2, sticky="w", padx=0, pady=(0, 10))
        ttk.Label(c_content, text="Usuario FTP", background=MINTY_PALETTE["bg_card"]).grid(row=1, column=0, sticky="w", pady=(2, 2), padx=(0, 10))
        ...
        ttk.Label(c_content, text="Password FTP", background=MINTY_PALETTE["bg_card"]).grid(row=1, column=1, sticky="w", pady=(2, 2), padx=(0, 10))
        ...
        ttk.Label(c_content, text="Ruta Local", background=MINTY_PALETTE["bg_card"]).grid(row=3, column=0, sticky="w", pady=(2, 2), padx=(0, 10))
        fr_ruta = tk.Frame(c_content, bg=MINTY_PALETTE["bg_card"])
        ...
        ttk.Label(c_content, text="Rango de Fechas (YYYY-MM-DD)", font=("Segoe UI", 10, "bold"), foreground=MINTY_PALETTE["primary"], background=MINTY_PALETTE["bg_card"]).grid(row=6, column=0, columnspan=2, sticky="w", padx=0, pady=(5, 5))
        ttk.Label(c_content, text="Fecha Inicio", background=MINTY_PALETTE["bg_card"]).grid(row=7, column=0, sticky="w", pady=(2, 2), padx=(0, 10))
        f_ini = tk.Frame(c_content, bg=MINTY_PALETTE["bg_card"])
        f_ini.grid(row=8, column=0, sticky="ew", padx=(0, 20))
        self.ent_ini = ttk.Entry(f_ini)
        self.ent_ini.pack(side="left", fill="x", expand=True)
        tk.Button(f_ini, text="📅", cursor="hand2", relief="flat", bg=MINTY_PALETTE["light"], fg=MINTY_PALETTE["primary"],
                  command=lambda: CalendarDialog(self.root, lambda d: [self.ent_ini.delete(0, tk.END), self.ent_ini.insert(0, d)])).pack(side="left", padx=2)
        self.ent_ini.insert(0, self.config.get('fecha_ini', '2025-01-01'))
        
        ttk.Label(c_content, text="Fecha Fin", background=MINTY_PALETTE["bg_card"]).grid(row=7, column=1, sticky="w", pady=(2, 2), padx=(0, 10))
        f_fin = tk.Frame(c_content, bg=MINTY_PALETTE["bg_card"])
        f_fin.grid(row=8, column=1, sticky="ew")
        self.ent_fin = ttk.Entry(f_fin)
        self.ent_fin.pack(side="left", fill="x", expand=True)
        tk.Button(f_fin, text="📅", cursor="hand2", relief="flat", bg=MINTY_PALETTE["light"], fg=MINTY_PALETTE["primary"],
                  command=lambda: CalendarDialog(self.root, lambda d: [self.ent_fin.delete(0, tk.END), self.ent_fin.insert(0, d)])).pack(side="left", padx=2)
        self.ent_fin.insert(0, self.config.get('fecha_fin', '2025-01-31'))

        row_actions = tk.Frame(main_container, bg=MINTY_PALETTE["bg_main"])
        ...
        self.frame_dashboard = tk.Frame(main_container, bg=MINTY_PALETTE["bg_main"])
        self.frame_dashboard.pack(fill="both", expand=True, pady=0)
        self.actualizar_dashboard()

    def crear_tab_archivos(self):
        main_container = tk.Frame(self.tab_archivos, bg=MINTY_PALETTE["bg_main"])
        main_container.pack(fill="both", expand=True, padx=20, pady=10)
        ...
        ttk.Label(c1, text="Nombre Archivo", background=MINTY_PALETTE["bg_card"]).grid(row=0, column=0, sticky="w", pady=(0, 5), padx=5)
        ...
        ttk.Label(c1, text="Ruta FTP", background=MINTY_PALETTE["bg_card"]).grid(row=0, column=1, sticky="w", pady=(0, 5), padx=5)
        ...
        self.tree_files.tag_configure("even", background=MINTY_PALETTE["bg_card"]); self.tree_files.tag_configure("odd", background=MINTY_PALETTE["bg_main"])
        ...
        fr_info = tk.Frame(main_container, bg=MINTY_PALETTE["light"], bd=1, relief="solid")
        fr_info.pack(fill="x"); fr_info.configure(highlightbackground="#bae6fd", highlightthickness=1)
        tk.Label(fr_info, text="⏬", bg=MINTY_PALETTE["light"], font=("Arial", 12)).pack(side="left", padx=10, pady=10)
        self.lbl_info_files_summary = tk.Label(fr_info, text=f"Archivos Configurados: {len(self.tree_files.get_children())}", justify="left", bg=MINTY_PALETTE["light"], fg=MINTY_PALETTE["primary"], font=("Segoe UI", 9))
        self.lbl_info_files_summary.pack(side="left", pady=10)

    def crear_tab_filtros(self):
        main_container = tk.Frame(self.tab_filtros, bg=MINTY_PALETTE["bg_main"])
        main_container.pack(fill="both", expand=True, padx=20, pady=10)

        fr_card_input = tk.Frame(main_container, bg=MINTY_PALETTE["bg_main"])
        ...
        ttk.Label(c1, text="Tabla", background=MINTY_PALETTE["bg_card"]).grid(row=0, column=0, sticky="w", pady=(0, 5), padx=5)
        ...
        ttk.Label(c1, text="Campo", background=MINTY_PALETTE["bg_card"]).grid(row=0, column=1, sticky="w", pady=(0, 5), padx=5)
        ...
        ttk.Label(c1, text="Valor", background=MINTY_PALETTE["bg_card"]).grid(row=0, column=2, sticky="w", pady=(0, 5), padx=5)
        ...
        ttk.Label(c1, text="Versión", background=MINTY_PALETTE["bg_card"]).grid(row=0, column=3, sticky="w", pady=(0, 5), padx=5)
        ...
        fr_btns = tk.Frame(c1, bg=MINTY_PALETTE["bg_card"]); fr_btns.grid(row=1, column=4, padx=5)
        def small_btn(txt, cmd, color=MINTY_PALETTE["primary"]):
            style = "Primary.TButton"
            if color == MINTY_PALETTE["success"]: style = "Success.TButton"
            b = RoundedButtonWrapper(fr_btns, text=txt, style=style, width=5, command=cmd)
            b.pack(side="left", padx=2)
            return b
        small_btn("✚", self.add_filtro, MINTY_PALETTE["success"]); small_btn("▲", self.move_up); small_btn("▼", self.move_down)

        fr_card_list = tk.Frame(main_container, bg=MINTY_PALETTE["bg_main"])
    ...
    def crear_metric_card(self, parent, icon, value, label, color=MINTY_PALETTE["primary"]):
        card = tk.Frame(parent, bg=MINTY_PALETTE["bg_card"], relief="flat", bd=0)
        inner = tk.Frame(card, bg=MINTY_PALETTE["bg_card"]); inner.pack(fill="both", expand=True, padx=20, pady=10)
        tk.Label(inner, text=icon, font=("Segoe UI", 24), bg=MINTY_PALETTE["bg_card"], fg=color).pack(side="left", padx=(0, 15))
        text_frame = tk.Frame(inner, bg=MINTY_PALETTE["bg_card"]); text_frame.pack(side="left", fill="both", expand=True)
        tk.Label(text_frame, text=str(value), font=("Segoe UI", 18, "bold"), bg=MINTY_PALETTE["bg_card"], fg=MINTY_PALETTE["dark"]).pack(anchor="w")
        tk.Label(text_frame, text=label, font=("Segoe UI", 9), bg=MINTY_PALETTE["bg_card"], fg="#64748b").pack(anchor="w")
        return card
    ...
    def actualizar_dashboard(self):
        ...
        self.crear_metric_card(grid_container, "💾", db_size, "Base de Datos", MINTY_PALETTE["info"] if db_exists else MINTY_PALETTE["danger"]).grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        self.crear_metric_card(grid_container, "📥", n_files, "Archivos Configurados", MINTY_PALETTE["success"]).grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        self.crear_metric_card(grid_container, "📋", n_filters, "Filtros Reporte", MINTY_PALETTE["warning"]).grid(row=0, column=2, padx=10, pady=10, sticky="nsew")
        
        info_frame = tk.Frame(grid_container, bg=MINTY_PALETTE["light"]); info_frame.grid(row=1, column=0, columnspan=3, sticky="ew", padx=10, pady=(0, 10))
        tk.Label(info_frame, text="FTP XM  ➔  📥 Descarga  ➔  💾 BD  ➔  📈 Visualizador", font=("Segoe UI", 11, "bold"), bg=MINTY_PALETTE["light"], fg=MINTY_PALETTE["primary"]).pack(side="right", padx=10)
