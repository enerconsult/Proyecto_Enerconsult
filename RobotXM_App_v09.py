# =============================================================================
#  ROBOT XM - SUITE INTEGRADA (v13 - FINAL + ESTADÍSTICAS Y EXPORTAR EXCEL)
#  Funcionalidades:
#  1. Descarga FTP Automática.
#  2. Base de Datos SQLite.
#  3. Reportes Excel.
#  4. VISUALIZADOR AVANZADO:
#     - Tipos de gráfico y colores.
#     - NUEVO: Panel de Estadísticas (Max, Min, Promedio).
#     - NUEVO: Botón para exportar los datos del gráfico a Excel.
#  5. Logo Corporativo.
# =============================================================================
...
class ModuloVisualizador:
    def __init__(self, parent_frame, config):
        ...
        self.datos_actuales = None # Variable para guardar datos y exportar
        ...
        self.cb_tabla.grid(row=0, column=1, padx=5); self.cb_tabla.bind("<<ComboboxSelected>>", self.al_seleccionar_tabla)
        ...
        self.cb_agregacion['values'] = ["Promedio", "Suma", "Máximo", "Mínimo"]; self.cb_agregacion.current(0)
        self.cb_agregacion.grid(row=4, column=1, sticky="w", padx=5)

        ttk.Label(frame_cfg, text="Tipo Gráfico:").grid(row=4, column=2, sticky="w", padx=5)
        self.cb_tipo = ttk.Combobox(frame_cfg, textvariable=self.var_tipo_grafico, state="readonly", width=15)
        self.cb_tipo['values'] = ["Línea", "Barras", "Área", "Dispersión"]
        self.cb_tipo.current(0)
        self.cb_tipo.grid(row=4, column=3, padx=5)

        ttk.Label(frame_cfg, text="Color:").grid(row=5, column=0, sticky="w")
        self.cb_color = ttk.Combobox(frame_cfg, textvariable=self.var_color_grafico, state="readonly", width=22)
        self.cb_color['values'] = list(COLORES_GRAFICO.keys())
        self.cb_color.current(0)
        self.cb_color.grid(row=5, column=1, sticky="w", padx=5)

        fr_fechas_viz = ttk.LabelFrame(frame_cfg, text="Rango de Fechas (Gráfico)")
        fr_fechas_viz.grid(row=6, column=0, columnspan=4, sticky="ew", pady=8)

        ttk.Label(fr_fechas_viz, text="Inicio (YYYY-MM-DD):").pack(side="left", padx=5)
        self.ent_fecha_ini = ttk.Entry(fr_fechas_viz, textvariable=self.var_fecha_ini, width=12)
        self.ent_fecha_ini.pack(side="left", padx=5)
        saved_ini = config.get('viz_fecha_ini', '2025-01-01')
        self.ent_fecha_ini.insert(0, saved_ini) 

        ttk.Label(fr_fechas_viz, text="Fin (YYYY-MM-DD):").pack(side="left", padx=5)
        self.ent_fecha_fin = ttk.Entry(fr_fechas_viz, textvariable=self.var_fecha_fin, width=12)
        self.ent_fecha_fin.pack(side="left", padx=5)
        saved_fin = config.get('viz_fecha_fin', datetime.today().strftime('%Y-%m-%d'))
        self.ent_fecha_fin.insert(0, saved_fin) 

        ttk.Button(frame_cfg, text="📊 GENERAR GRÁFICO", command=self.generar_grafico).grid(row=7, column=0, columnspan=2, pady=10, padx=5, sticky="ew")
        
        # --- NUEVO BOTÓN: EXPORTAR A EXCEL ---
        ttk.Button(frame_cfg, text="📥 EXCEL", command=self.exportar_datos_excel).grid(row=7, column=2, columnspan=2, pady=10, padx=5, sticky="ew")
        # -------------------------------------

        # --- NUEVO: PANEL DE ESTADÍSTICAS ---
        self.frame_stats = ttk.LabelFrame(self.frame_main, text="Resumen Estadístico", padding=5)
        self.frame_stats.pack(fill="x", padx=10, pady=5)
        
        self.lbl_stat_prom = ttk.Label(self.frame_stats, text="Promedio: --", font=('Arial', 9, 'bold'))
        self.lbl_stat_prom.pack(side="left", padx=15)
        
        self.lbl_stat_max = ttk.Label(self.frame_stats, text="Máximo: --", font=('Arial', 9, 'bold'), foreground="green")
        self.lbl_stat_max.pack(side="left", padx=15)
        
        self.lbl_stat_min = ttk.Label(self.frame_stats, text="Mínimo: --", font=('Arial', 9, 'bold'), foreground="red")
        self.lbl_stat_min.pack(side="left", padx=15)
        
        self.lbl_stat_sum = ttk.Label(self.frame_stats, text="Total (Suma): --", font=('Arial', 9, 'bold'), foreground="blue")
        self.lbl_stat_sum.pack(side="left", padx=15)
        # ------------------------------------
    ...
    def toggle_campo_valor(self):
        if self.var_es_horario.get():
            self.cb_campo_valor.configure(state="disabled"); self.lbl_valor.configure(text="6. (Modo 24 Horas)")
        else:
            self.cb_campo_valor.configure(state="readonly"); self.lbl_valor.configure(text="6. Variable a Graficar:")
    ...
    def al_seleccionar_tabla(self, event):
        ...
        else: self.cb_version.set("N/A"); self.cb_version['values'] = []
        conn.close()
        
        cols_horarias = [str(i) for i in range(24)]
        es_horario = all(h in cols for h in cols_horarias)
        self.var_es_horario.set(es_horario); self.toggle_campo_valor()
        ...
        self.cb_campo_filtro1['values'] = candidatos; self.cb_campo_filtro2['values'] = candidatos
    ...
    def _cargar_valores_filtro(self, var_campo, widget_cb):
        ...
        try:
            ...
            widget_cb['values'] = vals; widget_cb.set('') 
        except: pass
    ...
    def generar_grafico(self):
        ...
        operacion = self.var_agregacion.get(); es_24h = self.var_es_horario.get()
        ...
        try:
            ...
            self.lbl_stat_max.config(text=f"Máximo: {val_max:,.2f}")
            self.lbl_stat_min.config(text=f"Mínimo: {val_min:,.2f}")
            self.lbl_stat_sum.config(text=f"Total: {val_sum:,.2f}")
            # -----------------------------
            ...
            self.dibujar_plot(self.datos_actuales, titulo_grafico, tipo_grafico, color_hex)

        except Exception as e: messagebox.showerror("Error", f"{e}")

    def exportar_datos_excel(self):
        """Exporta los datos que se están viendo en pantalla a Excel."""
        if self.datos_actuales is None:
            messagebox.showwarning("Sin Datos", "Primero genera un gráfico para poder exportar.")
            return
        ...
        try:
            # Convertir Serie a DataFrame para exportar bonito
            df_export = self.datos_actuales.reset_index()
            df_export.columns = ['Fecha', 'Valor']
            df_export['Fecha'] = df_export['Fecha'].dt.date # Quitar hora
    ...
    def dibujar_plot(self, serie, titulo, tipo, color):
        for widget in self.frame_plot.winfo_children(): widget.destroy()
        fig = Figure(figsize=(4,4), dpi=100)
        ax = fig.add_subplot(111)
        
        if tipo == "Línea":
            ax.plot(serie.index, serie.values, marker='o', linestyle='-', markersize=4, color=color)
        elif tipo == "Barras":
            ax.bar(serie.index, serie.values, color=color, alpha=0.8, width=0.6)
        elif tipo == "Área":
            ax.fill_between(serie.index, serie.values, color=color, alpha=0.5)
            ax.plot(serie.index, serie.values, color=color, linewidth=1.5)
        elif tipo == "Dispersión":
            ax.scatter(serie.index, serie.values, color=color, s=25)
...
class AplicacionXM:
    ...
    def crear_tab_filtros(self):
        ...
        ttk.Button(fr_in, text="+", width=3, command=self.add_filtro).pack(side="left", padx=2)
        ttk.Button(fr_in, text="-", width=3, command=self.del_filtro).pack(side="left", padx=2)
        ttk.Separator(fr_in, orient="vertical").pack(side="left", padx=5, fill="y")
        ttk.Button(fr_in, text="⬆️", width=3, command=self.move_up).pack(side="left", padx=2)
        ttk.Button(fr_in, text="⬇️", width=3, command=self.move_down).pack(side="left", padx=2)
