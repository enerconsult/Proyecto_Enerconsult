# =============================================================================
#  ROBOT XM - SUITE INTEGRADA (v07 - FINAL + REORDENAR FILTROS)
#  Funcionalidades:
...
#  5. NUEVO: Botones para subir/bajar el orden de los reportes.
# =============================================================================
...
class AplicacionXM:
    ...
    def crear_tab_filtros(self):
        ...
        # --- BOTONES DE CONTROL (+, -, SUBIR, BAJAR) ---
        ...
        # Botones de Reordenar con Flechas
    ...
    # --- NUEVAS FUNCIONES PARA MOVER FILAS ---
    def move_up(self):
        ...
        for item_id in selection:
            ...
            if idx > 0:
                # move(item, parent, index) - parent es "" para el nivel raíz
                self.tree_filtros.move(item_id, "", idx - 1)
                self.tree_filtros.see(item_id) # Mantener visible

    def move_down(self):
        """Mueve la fila seleccionada una posición hacia abajo."""
        selection = self.tree_filtros.selection()
        if not selection: return
        # Iteramos al revés por si hay selección múltiple (para evitar problemas de índice)
        for item_id in reversed(selection):
            idx = self.tree_filtros.index(item_id)
            last_idx = len(self.tree_filtros.get_children()) - 1
            if idx < last_idx:
                # move inserta *antes* del índice, así que para bajar usamos idx + 1
                self.tree_filtros.move(item_id, "", idx + 1)
                self.tree_filtros.see(item_id)
    # -----------------------------------------
    ...
    def get_config(self):
        # El orden se toma directamente del Treeview, respetando los movimientos hechos por el usuario
        return {
    ...
    def guardar_config(self):
        try:
            ...
            print("✅ Configuración guardada (incluyendo nuevo orden).")
    ...
    def run_reporte(self):
        # El reporte leerá la configuración en el orden visual actual del Treeview
